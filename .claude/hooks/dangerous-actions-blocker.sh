...
if [[ "$TOOL_NAME" == "Bash" ]]; then
    ...
    if echo "$SCAN" | grep -qE 'rm\s+(-[a-zA-Z]+\s+)*-?[a-zA-Z]*[rR][a-zA-Z]*\s+[^;&|]*(\.venv|querysource/?\s*$|\.git(/|\s|$)|\.claude/worktrees)'; then
        block "Recursive delete of a repo-critical path (.venv, querysource/, .git, .claude/worktrees)." \
