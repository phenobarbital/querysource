...
(`SkillsDirectoryLoader` / `SkillFileRegistry` in ai-parrot's `parrot/skills/`), so a
...
`CLAUDE.md`, `.agent/CONTEXT.md`, `sdd/WORKFLOW.md` and every
...
grep -inE 'ignore (all|previous)|disregard|you are now|new role|system prompt|skip (the )?review|always approve|do not ask' \
  CLAUDE.md .agent/CONTEXT.md sdd/WORKFLOW.md .claude/rules/*.md 2>/dev/null
