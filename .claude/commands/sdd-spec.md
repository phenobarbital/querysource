...
Scaffold a new Feature Specification for QuerySource using the SDD methodology.
...
2. **Route the answer into the spec body where the decision actually
   ...
   - "Binary overflow path declared in `querysource/conf.py`" → add the config
...
- Identify related existing components (BaseProvider, externalProvider, QS, QueryConnection, AbstractParser, Output, etc.).
...
3. **Verify all imports**: confirm that `from querysource.X import Y` resolves by
