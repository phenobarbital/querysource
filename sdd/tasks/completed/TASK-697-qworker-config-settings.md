...
**Date**: 2026-05-26
**Notes**: Added QWORKER_HOST, QWORKER_PORT, QWORKER_TIMEOUT to conf.py after the Airtable block, following the same navconfig.config.get()/getint() pattern. QWORKER_HOST defaults to None (remote not configured), QWORKER_PORT to 8888 (matches qworker default), QWORKER_TIMEOUT to 60s.
