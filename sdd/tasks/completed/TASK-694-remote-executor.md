...
**Date**: 2026-05-26
**Notes**: RemoteExecutor was already created in executors.py during TASK-693 (same file per spec). TASK-694 added the test file. QClient is imported lazily inside execute(). Transport errors (ConnectionError, TimeoutError, OSError) are wrapped in QueryException with worker address. qworker-side errors (SlugNotFound, etc.) propagate as-is.
