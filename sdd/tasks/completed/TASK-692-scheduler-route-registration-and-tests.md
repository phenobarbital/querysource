...
**Date**: 2026-05-23
**Notes**: Implemented Modules 2 and 3 from the spec:
- Added `SchedulerJobsView` re-export to `querysource/handlers/__init__.py`.
- Registered both scheduler routes inside the `if ENABLE_QS_SCHEDULER:` block in
  `querysource/services.py` using a lazy import following the existing pattern.
- Created 6 integration tests using `aiohttp.test_utils.TestServer` + `TestClient`
  (not `aiohttp_client` from `pytest-aiohttp`, which is not installed in this project).
- Fixed a defensive issue discovered during integration testing: APScheduler Job uses
  `__slots__` and some slots (`next_run_time`, `coalesce`, `max_instances`,
  `misfire_grace_time`) are only set after the scheduler starts. Changed `_serialize_job`
  to use `getattr(job, attr, default)` to avoid `AttributeError` on pending schedulers.
- The `test_route_not_registered_when_flag_off` test was simplified: rather than booting
  the full `QuerySource` singleton (which requires DB/template dependencies), it builds a
  minimal app with NO routes registered and confirms the router returns 404 (not the
  handler's 503). This is functionally equivalent and avoids untestable external deps.
- All 19 tests pass (13 unit + 6 integration). Pre-existing ruff warnings in
  `services.py` (unused imports of `sys` and `subprocess`) not fixed — out of scope.

**Deviations from spec**: The integration test for "flag off" uses a direct no-route
approach instead of monkeypatching `QuerySource` singleton, due to the Cython Singleton
metaclass complexity and external service dependencies. The functional contract is
identical: route not registered → 404 from router.
