...
**Date**: 2026-05-26
**Notes**: Modified ThreadQuery to accept optional remote_config parameter. Replaced the QueryObject import in query.py with LocalExecutor/RemoteExecutor/RemoteConfig imports from executors.py. Updated fetch() to delegate to self._executor.execute(). Updated slug property to be purely dict-based (no longer depends on QueryObject replacement). Backward compatible — existing callers without remote_config use LocalExecutor.
