...
**Date**: 2026-05-20
**Notes**: Added import json, updated import to include scheduled_multiqs_job,
extended SELECT to include query_raw, added inline provider branch in
_load_scheduled_queries with WARN-on-misconfig and DEBUG-on-output-subkey.
Updated module docstring. Added 5 new unit tests (all pass). No linting errors.
Note: caplog tests were adapted to use MagicMock logger call_args_list instead
of caplog records, since the fixture replaces qs.logger with a MagicMock.
