...
**Date**: 2026-05-26
**Notes**: Added RemoteConfig and QWORKER_* imports to multi/__init__.py. Added _remote_queries list to MultiQS.__init__. Modified the self._queries dispatch loop to pop remote/worker keys, resolve worker address (per-query worker key -> QWORKER_HOST/PORT -> DriverError), build RemoteConfig, and pass it to ThreadQuery. Backward compatible: queries without remote key use remote_config=None (LocalExecutor path).
