...
**Date**: 2026-05-23
**Notes**: Implemented `SchedulerJobsView(BaseView)` in `querysource/handlers/scheduler.py`
with the module-level `_kind_from_id` function and all required methods (`_get_scheduler`,
`_serialize_job`, `get`). All 13 unit tests pass. Ruff check passes.

The worktree required copying compiled Cython `.so` extensions from the main repo to allow
the test suite to run (the worktree only contains git-tracked source files, not compiled
artifacts). This is an environment constraint and does not affect the implementation.
