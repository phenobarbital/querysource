"""Backward compatibility tests for refactored FileSource (TASK-645)."""
import asyncio
...
from querysource.queries.multi.sources.file import FileSource


class TestFileSourceRefactor:
    def test_inherits_thread_source(self):
        assert issubclass(FileSource, ThreadSource)

    def test_has_fetch_method(self):
        assert hasattr(FileSource, 'fetch')

    def test_run_is_not_overridden(self):
        # run() should come from ThreadSource, not FileSource
        assert 'run' not in FileSource.__dict__

    def test_csv_file_produces_dataframe(self, tmp_path):
        csv_file = tmp_path / "test.csv"
        csv_file.write_text("col1,col2\n1,a\n2,b")
        queue = asyncio.Queue()
        t = FileSource("test_csv", {"path": str(csv_file), "mime": "text/csv"}, None, queue)
        t.start()
        t.join()
    ...
    def test_csv_file_content_is_correct(self, tmp_path):
        ...
        queue = asyncio.Queue()
        t = FileSource("data", {"path": str(csv_file), "mime": "text/csv"}, None, queue)
        t.start()
        t.join()
        assert t.exc is None
        loop = asyncio.new_event_loop()
    ...
    def test_gz_csv_file_produces_dataframe(self, tmp_path):
        ...
        with gzip.open(gz_file, 'wb') as f:
            f.write(csv_content)
        queue = asyncio.Queue()
        t = FileSource("gz_test", {"path": str(gz_file), "mime": "text/csv"}, None, queue)
        t.start()
        t.join()
        assert t.exc is None
        loop = asyncio.new_event_loop()
    ...
    def test_constructor_signature_preserved(self, tmp_path):
        """Verify FileSource(name, file_options, request, queue) still works."""
